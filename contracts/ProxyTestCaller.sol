 ...
 * contract through an external call, demonstrating how a consumer contract
 * integrates.
 *
 * Note: the proxy contract itself becomes `msg.sender` of the forwarded call,
 * so the emitted event's `sender` (and therefore the request ID and the MPC
 * key-derivation predecessor) is THIS contract's address, not the originating
 * wallet. This matches how real consumer contracts own their derived MPC keys.
 */
